...
{
    resize(850, 550);
    setWindowTitle(tr("DonCoin") + " - " + tr("Wallet"));
#ifndef Q_WS_MAC
...
void BitcoinGUI::setMining(bool mining, int hashrate)
{
    if (mining)
    {
        ...
        labelMiningIcon->setToolTip(tr("Mining DonCoin at %1 hashes per second").arg(hashrate));
    }
    else
    {
        labelMiningIcon->setPixmap(QIcon(":/icons/mining_inactive").pixmap(STATUSBAR_ICONSIZE,STATUSBAR_ICONSIZE));
        labelMiningIcon->setToolTip(tr("Not mining DonCoin"));
    }
}
